...
(function () {
    ...
    iTable.prototype.bulidHtml = function() {
        ...
        if (_options.toolbar.auto) {
            ...
            if ($.inArray("re_load", _options.toolbar.exclude) < 0) {
                html.push(
            ...
            if ($.inArray("reset", _options.toolbar.exclude) < 0) {
                html.push(
            ...
            if ($.inArray("batch_remove", _options.toolbar.exclude) < 0) {
                html.push(
            ...
            if ($.inArray("dt_insert", _options.toolbar.exclude) < 0) {
                html.push(
    ...
    iTable.prototype.listen = function () {
    	var _this = this;
    	if ($.inArray("re_load", _options.toolbar.exclude) < 0) {
    		$("#" + _options.toolbar.id).find("#btn_re_load").bind("click", function () {
                _this.re_load();
            });
        }

        if ($.inArray("reset", _options.toolbar.exclude) < 0) {
            $("#" + _options.toolbar.id).find("#btn_reset").bind("click", function () {
                _this.reset();
            });
        }

        if ($.inArray("batch_remove", _options.toolbar.exclude) < 0) {
            $("#" + _options.toolbar.id).find("#btn_batch_remove").bind("click", function () {
                _this.batch_remove();
            });
        }

        if ($.inArray("dt_insert", _options.toolbar.exclude) < 0) {
            $("#" + _options.toolbar.id).find("#btn_dt_insert").bind("click", function () {
                _this.dt_insert();
            });
        }
    }
    ...
    iTable.prototype.dt_delete_button = function (row) {
        var deleteO = '<a class="btn btn-warning btn-sm" href="#" title="删除"  mce_href="#" ' +
            'onclick="iTable.remove(\'' + row.id + '\')"><i class="fa fa-remove"></i></a> ';
        return deleteO;
    };
    ...
    iTable.prototype.dt_detail_button = function (row) {
        var detailO = '<a class="btn btn-success btn-sm" href="#" title="详情"  mce_href="#" onclick="iTable.dt_detail(\'' + row.id + '\')"><i class="fa fa-info-circle"></i></a> ';
        return detailO;
    };
    ...
    $.fn[pluginName] = function (options, args) {
        ...
        this.each(function () {
            ...
            if (typeof options === 'string') {
            	...
            	result = $.data(this, pluginName, new iTable(this, $.extend(true, {}, options)));
            }
        });
