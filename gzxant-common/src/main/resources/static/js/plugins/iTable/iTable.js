...
(function() {
	...
	var _default = {
		toolbar: {
			...
			btn: {
				batch_remove: {
					url : url + "delete",
					open_type : 1 //open_type ： {1 ： 弹出框， 2：跳转页面}
				},
				dt_insert: {
					url: url + "insert",
					open_type : 1 //open_type ： {1 ： 弹出框， 2：跳转页面}
	...
	iTable.prototype.bulidHtml = function() {
		...
		if(_options.toolbar.auto) {
			...
			if($.inArray("re_load", _options.toolbar.btn.exclude) < 0) {
				html
					.push(
						'<button type="button" id="btn_re_load" class="btn btn-success" style="margin-left: 10px;">',
						'<i class="fa fa-search" aria-hidden="true"></i> 查询',
			...
			if($.inArray("reset", _options.toolbar.btn.exclude) < 0) {
				html
					.push(
			...
			if($.inArray("batch_remove", _options.toolbar.btn.exclude) < 0) {
				html
					.push(
			...
			if($.inArray("dt_insert", _options.toolbar.btn.exclude) < 0) {
				html
					.push(
	...
	iTable.prototype.remove = function(id) {
		...
		layer.confirm('确定要删除选中的记录？', {
			...
			$.ajax({
				url: _options.table.btn.remove.url,
				type: "POST",
				data: {
					'ids': id
				},
	...
	iTable.prototype.batch_remove = function() {
		...
		layer.confirm("确认要删除选中的'" + rows.length + "'条数据吗?", {
			...
			$.ajax({
				...
				url: _options.toolbar.btn.batch_remove.url,
				success: function(r) {
					_this.dataTable_rep_message(r)
				}
			});
	...
	iTable.prototype.load_data = function() {
		...
		this._element.bootstrapTable({
			...
			url: option.table.data.url, // 服务器数据的加载地址
			//showRefresh : true,
			//showToggle : true,
			//showColumns : true,
			iconSize: 'outline',
			...
			// //发送到服务器的数据编码类型
			pageSize: option.table.page.pageSize, // 如果设置了分页，每页数据条数
			pageNumber: option.table.page.pageNumber, // 如果设置了分布，首页页码
			//search : true, // 是否显示搜索框
			...
			queryParams: function(params) {
				...
				var p = {
					"pageNumber": option.table.page.pageNumber,
					"pageSize": option.table.page.pageSize,
					"searchParams": searchParams,
					"sorts": option.table.page.sorts
				};
			...
			// sortOrder.
			// 返回false将会终止请求
			columns: option.table.data.columns
		});
	}
	...
	iTable.prototype.export_data = function() {
		...
		$.ajax({
			type: "POST", //提交方式
			url: option.toolbar.btn.export_data.url, // 服务器数据的加载地址
			dataType: "json",
